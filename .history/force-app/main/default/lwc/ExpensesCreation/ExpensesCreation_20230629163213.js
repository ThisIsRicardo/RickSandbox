...
@track currentExpense = {
 ...
 // New function to calculate total amounts for approved and pending expenses
  calculateExpensesTotal() {
    this.totalApprovedExpenses = 0;
    this.totalPendingExpenses = 0;
    this.expenses.forEach(expense => {
      if (expense.Stage__c === 'Pending Approval') {
        this.totalPendingExpenses += expense.Amount_Spent__c;
      } else if (expense.Submit_for_Approval__c === false) {
        this.totalApprovedExpenses += expense.Amount_Spent__c;
      }
    });
  }
