...
@track currentExpense = {
  ...
  @track selectedColumns = [
    ...
    { label: 'Category', fieldName: 'Subcategory__r.Name' },
  ...
  getExpensesforDataTable() {
    ...
    getExpenses({ statementId: this.statementId })
.then(result => {
    // For each expense, modify the related fields to a flat string that can be displayed in the data table
    this.expenses = result.map(expense => {
        return {
            ...expense,
            Associated_Credit_Card__c: expense.Associated_Credit_Card__r ? expense.Associated_Credit_Card__r.Name : '',
            Credit_Card_Statement__c: expense.Credit_Card_Statement__r ? expense.Credit_Card_Statement__r.Name : '',
            Associated_Team_Member__c: expense.Associated_Team_Member__r ? expense.Associated_Team_Member__r.Name : '',
            Subcategory__c: expense.Subcategory__r ? expense.Subcategory__r.Name : '',
            License__c: expense.License__r ? expense.License__r.Name : '',
            Course__c: expense.Course__r ? expense.Course__r.Name : '',
        };
    });
})
thi.calculateExpensesTotal();
      .catch(error => {
        console.error('Error in updateExpenseWithCreditCardStatement:', error);
      })
 ...
 calculateExpensesTotal() {
  ...
  this.expenses.forEach(expense => {
    if (expense.Stage__c === 'Pending Approval') {
      tempTotalAmountSpentPending += expense.Amount_Spent__c;
    } else if (expense.Stage__c === 'Approved') {
      tempTotalAmountSpentApproved += expense.Amount_Spent__c;
    }
  });
...
async handleSaveExpenses() {
  try {
    await saveExpenses({ expense: this.currentExpense })
    this.dispatchEvent(
      new ShowToastEvent({
        title: 'Success',
