...
parseAmount(amountString) {
  ...
  let noCommas = amountString.replace(/,/g, '');

  // Replace decimal comma with dot, if present
  let noDecimalComma = noCommas.replace(',', '.');

  return parseFloat(noDecimalComma);
}


handleAmountChange(event) {
  let inputValue = event.target.value;
  let normalizedInputValue = parseAmount(inputValue);
  this.currentExpense.Amount_Spent__c = normalizedInputValue;
