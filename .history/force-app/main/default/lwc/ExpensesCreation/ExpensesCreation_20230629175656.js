...
async deleteExpense(row) {
  const expenseId = row.expenseData.Id; // You get Id from expenseData now
  try {
    await deleteExpense({ expenseId: expenseId });
    // directly manipulate the expenses array to reflect the change immediately
    this.expenses = this.expenses.filter(expense => expense.expenseData.Id !== expenseId);
    this.dispatchEvent(
      new ShowToastEvent({
        title: 'Success',
