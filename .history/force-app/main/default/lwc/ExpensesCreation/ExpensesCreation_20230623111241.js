...
handleAddExpense() {
  ...
  for (let i = 0; i < this.transactions.length; i++) {
    if (this.transactions[i].Submit_for_Approval__c === false) {
      this.totalAmountSpentApproved += parseFloat(this.transactions[i].Amount_Spent__c.replace(',', '.'));
  }
  // Else if Loop for expenses with submit for approval as true

  else if (this.transactions[i].Submit_for_Approval__c === true) {
    this.totalAmountSpentPending += parseFloat(this.transactions[i].Amount_Spent__c.replace(',', '.'));
}
