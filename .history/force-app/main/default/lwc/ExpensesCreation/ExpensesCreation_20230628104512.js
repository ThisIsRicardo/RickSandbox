...
handleRecordTypeChange(event) {
  ...
  handleDateChange(event) {
    ...
    this.expense.Transaction_Date__c = transactionDate

    // Fetch the budget if the amount and subcategory are set
    if (
      this.expense.Amount_Spent__c &&
      this.expense.Subcategory__c
    ) {
  ...
  handleCreditCardStatementChange(event) {
    ...
    // Get expenses for the selected credit card statement
    this.getExpensesForStatement();
    
    // Update expenses with credit card statement
    this.updateExpensesWithCreditCardStatement();
  }
  
  getExpensesForStatement() {
    if (this.creditCardStatement) {
      getExpenses({ statementId: this.creditCardStatement })
        .then(result => {
  ...
  switch (this.transactionType) {
    ...
    case 'Other Expense':
      ...
      break;
        case 'Multiple Team Members':
          // Make sure that all the required fields are filled in before saving.
          if (this.expenses[0].Transaction_Date__c === '' || this.expenses[0].Name === '' ||this.expenses[0].Subcategory__c === '' || this.expenses[0].Amount_Spent__c === '' || this.expenses[0].Associated_Credit_Card__c === '' || this.expenses[0].Team_Members_ID_s__c === '' || this.expenses[0].Receipt_Type__c === '') {
            this.dispatchEvent(
              new ShowToastEvent({
                title: 'Missing Required Fields',
                message: 'Please, fill in all the required fields.',
        ...
        case 'None':
          // Make sure that all the required fields are filled in before saving.
          if (this.expenses[0].Transaction_Date__c === '' || this.expenses[0].Name === '' ||this.expenses[0].Subcategory__c === '' || this.expenses[0].Amount_Spent__c === '' || this.expenses[0].Associated_Credit_Card__c === '' || this.expenses[0].Receipt_Type__c === '') {
            this.dispatchEvent(
              new ShowToastEvent({
                title: 'Missing Required Fields',
                message: 'Please, fill in all the required fields.',
