...
async handleSaveExpenses() {
  ...
  // Making sure that each transaction type has the required fields filled
  switch (this.transactionType) {
    case 'License':
      if (!this.currentExpense.License__c) {
        this.dispatchEvent(
          new ShowToastEvent({
            title: 'Error',
      ...
      break;
    case 'Event':
      if (!this.currentExpense.Course__c) {
        this.dispatchEvent(
          new ShowToastEvent({
            title: 'Error',
    ...
    case 'Other Expense':
      switch (this.otherExpenseType) {
        case 'One': 
          if (!this.currentExpense.Associated_Team_Member__c) {
            this.dispatchEvent(
              new ShowToastEvent({
                title: 'Error',
          ...
          break;
        case 'Multiple Team Members':
          if (!this.currentExpense.selectedTeamMembers) {
            this.dispatchEvent(
              new ShowToastEvent({
                title: 'Error',
                message: 'Please, select at least two team member.',
                variant: 'error'
              })
            );
            return;
