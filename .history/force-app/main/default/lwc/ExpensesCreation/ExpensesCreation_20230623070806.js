...
async fetchSubcategoryName(Id) {
    try {
        const subcategory = await getRecordUi({
            recordIds: [Id],
            layoutTypes: ['Compact'],
            modes: ['View']
        });
        return subcategory.records[Id].fields.Name.value;
    } catch (error) {
...
handleCreditCardSelect(event) {
  ...
  getCreditCardName({ creditCardId: selectedCreditCardId })
    .then(result => {
      let expenseWrapper = new ExpenseWrapper(this.newExpense);
...
handleStatementSelect(event) {
  ...
  getStatementName({ statementId: selectedStatementId })
    .then(result => {
      let expenseWrapper = new ExpenseWrapper(this.newExpense);
...
handleTeamMemberSelect(event) {
  ...
  getTeamMemberName({ teamMemberId: selectedTeamMemberId })
    .then(result => {
      let expenseWrapper = new ExpenseWrapper(this.newExpense);
...
handleLicenseSelect(event) {
  ...
  getLicenseName({ licenseId: selectedLicenseId })
    .then(result => {
      let expenseWrapper = new ExpenseWrapper(this.newExpense);
...
handleCourseSelect(event) {
  ...
  getCourseName({ courseId: selectedCourseId })
    .then(result => {
      let expenseWrapper = new ExpenseWrapper(this.newExpense);
