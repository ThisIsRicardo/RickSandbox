  ...
  switch (this.transactionType) {
    ...
    case 'Other Expense':
      switch (this.otherExpenseType) {
        case 'One':
          // Make sure that all the required fields are filled in before saving.
          if (this.expenses[0].Transaction_Date__c === '' || this.expenses[0].Name === '' ||this.expenses[0].Subcategory__c === '' || this.expenses[0].Amount_Spent__c === '' || this.expenses[0].Associated_Credit_Card__c === '' || this.expenses[0].Assoc === '' || this.expenses[0].Receipt_Type__c === '') {
            this.dispatchEvent(
              new ShowToastEvent({
                title: 'Missing Required Fields',
                message: 'Please, fill in all the required fields.',
