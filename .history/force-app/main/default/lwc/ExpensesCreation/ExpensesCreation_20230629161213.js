...
async handleSaveExpenses() {
  try {
    ...
    this.calculateExpensesTotal(); // call after adding a new expense
  } catch (error) {
...
async deleteExpense(row) {
  ...
  try {
    ...
    this.calculateExpensesTotal(); // call after deleting an expense
  } catch (error) {
    // Handle error
    this.dispatchEvent(
      new ShowToastEvent({
...
calculateExpensesTotal() {
  let totalAmountSpentApproved = 0;
  let totalAmountSpentinPending = 0;

  this.expenses.forEach(expense => {
    if(expense.Submit_for_Approval__c) {
      totalAmountSpentinPending += parseFloat(expense.Amount_Spent__c);
    } else {
      totalAmountSpentApproved += parseFloat(expense.Amount_Spent__c);
    }
  });

  this.totalAmountSpentApproved = totalAmountSpentApproved;
  this.totalAmountSpentinPending = totalAmountSpentinPending;
}
