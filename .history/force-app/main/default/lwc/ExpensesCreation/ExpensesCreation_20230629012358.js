...
export default class ExpenseCreation extends LightningElement {
  ...
  async refreshDataTable(){
    awaitgetExpenses({ statementId: this.currentExpense.Credit_Card_Statement__c })
    .then(result => {
  ...
  async connectedCallback() {
    ...
    if(this.value == 'Staff Client Management'){
        await getRelatedStaffTeamMembers({accountId : this.value})
        .then((data)=>{
            this.teamMemberOptions = data;
            this.weightValidationError = false;
            this.notNullValidationError = false;
        }).catch((error)=>{
            this.dispatchEvent(
                new ShowToastEvent({
...
handleSaveExpenses() {
  ...
  saveExpenses({ expense: this.currentExpense }) // remember to update the method name to saveExpense in your Apex class
    .then(result => {
      ...
      // Update the expenses data table after saving a new expense
      this.updateExpenseWithCreditCardStatement();
    })
    .catch(error => {
