...
handleAddExpense() {
  ...
  switch (this.transactionType) {
    ...
    case 'Event':
      // Make sure that all the required fields are filled in before saving.
      if (this.expenses[0].Transaction_Date__c === '' || this.expenses[0].Name === '' ||this.expenses[0] this.expenses[0].Amount_Spent__c === '' || this.expenses[0].Associated_Credit_Card__c === '' || this.expenses[0].License__c === '' || this.expenses[0].Receipt_Type__c === '') {
        this.dispatchEvent(
          new ShowToastEvent({
            title: 'Missing Required Fields',
            message: 'Please, fill in all the required fields.',
