...
handleCreditCardStatementChange(event) {
  // Update the credit card statement value
  this.currentExpense.Credit_Card_Statement__c = event.detail.value;

  // Fetch the updated expenses based on the selected credit card statement
  getExpenses({ statementId: event.detail.value })
    .then(result => {
      // Update the expenses data table with the fetched data
...
handleSaveExpenses() {
  // Add here validations and saving logic similar to your previous implementation

  saveExpenses({ expense: this.currentExpense })
    .then(result => {
      ...
      // Update the expenses data table after saving the new expense
      this.updateExpensesData();
    })
    .catch(error => {
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: 'Error',
          message: 'An error occurred while trying to save the expense.',
          variant: 'error'
        })
      );
    });
...
updateExpensesData() {
  // Fetch the updated expenses data and update the data table
  getExpenses()
    .then(result => {
      this.expenses = result;
    })
...
resetCurrentExpense() {
  // Reset the currentExpense object to its initial state
  this.currentExpense = {
    Name: '',
    Transaction_Date__c: '',
    Amount_Spent__c: '',
