// expenseGrid.js
import { LightningElement, track, wire, getRecord } from 'lwc'
...
import fetchSubcategoryName from '@salesforce/apex/ExpenseController.fetchSubcategoryName'
import fetchCreditCardName from '@salesforce/apex/ExpenseController.fetchCreditCardName'
import fetchStatementName from '@salesforce/apex/ExpenseController.fetchStatementName'
import fetchTeamMemberName from '@salesforce/apex/ExpenseController.fetchTeamMemberName'
import fetchLicenseName from '@salesforce/apex/ExpenseController.fetchLicenseName'
import fetchCourseName from '@salesforce/apex/ExpenseController.fetchCourseName'
...
export default class ExpenseCreation extends LightningElement {
  ...
  async fetchSubcategoryName(Id) {
    try {
      const subcategory = await getRecord({recordId: Id, fields: ['Subcategory__c.Name']});
      return subcategory.fields.Name.value;
    } catch (error) {
  ...
  async fetchCreditCardName(creditCardId) {
    try {
      const creditCard = await getRecord({recordId: creditCardId, fields: ['CreditCard__c.Name']});
      return creditCard.fields.Name.value;
    } catch (error) {
  ...
  async fetchStatementName(statementId) {
    try {
      const statement = await getRecord({recordId: statementId, fields: ['Statement__c.Name']});
      return statement.fields.Name.value;
    } catch (error) {
  ...
  async fetchTeamMemberName(teamMemberId) {
    try {
      const teamMember = await getRecord({recordId: teamMemberId, fields: ['TeamMember__c.Name']});
      return teamMember.fields.Name.value;
    } catch (error) {
  ...
  async fetchLicenseName(licenseId) {
    try {
      const license = await getRecord({recordId: licenseId, fields: ['License__c.Name']});
      return license.fields.Name.value;
    } catch (error) {
  ...
  async fetchCourseName(courseId) {
    try {
      const course = await getRecord({recordId: courseId, fields: ['Course__c.Name']});
      return course.fields.Name.value;
    } catch (error) {
...
getRowOptions(rowId) {
  ...
  handleSubcategorySelect(event) {
    ...
    // use the ID to fetch the record name
    fetchSubcategoryName({ subcategoryId: selectedSubcategoryId })
    .then(result => {
        ...
        // now update the table with this.transactions
    })
    .catch(error => {
        // handle error...
    });
