...
updateExpenseWithCreditCardStatement() {
  ...
  .then((result) => {
    ...
    this.totalAmountSpentApproved = this.expenses.reduce((total, expense) => {
      if (expense.Stage__c === 'Approved') {
        return total + expense.Amount_Spent__c;
      } else {
        return total;
      }
    }, 0);

    this.totalAmountSpentPending = this.expenses.reduce((total1, expense) => {
      if (expense.Stage__c === 'Pending Approval') {
        return total1 + expense.Amount_Spent__c;
      } else {
        return total1;
      }
    }, 0);
...
handleSaveExpenses() {
  saveExpenses({ expense: this.currentExpense }) // remember to update the method name to saveExpense in your Apex class
    .then(result => {
      ...
      // Delay of 2 seconds before refreshing the data table.
    })
}
