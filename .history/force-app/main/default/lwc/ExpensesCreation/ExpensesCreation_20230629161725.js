...
@track currentExpense = {
  ...
  calculatexpensesTotal(){
    //loop through each one of the expenses to get a total of amount spent approved and amount spent not approved
    this.totalAmountSpentApproved = 0;
    this.totalAmountSpentNotApproved = 0;
    for (let i = 0; i < this.expenses.length; i++) {
      if (this.expenses[i].Stage == 'Approved') {
        this.totalAmountSpentApproved += this.expenses[i].Amount_Spent__c;
      } else {
        this.totalAmountSpentNotApproved += this.expenses[i].Amount_Spent__c;
      }
    }
  }
